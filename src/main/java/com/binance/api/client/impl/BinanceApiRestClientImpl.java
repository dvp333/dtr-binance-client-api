...
public class BinanceApiRestClientImpl implements BinanceApiRestClient {
	...
	@Override
	public List<TickerStatistics> getPriceChangeStatistics(
        String symbol,
		String symbols,
        String windowSize,
        String type) {
			return executeSync(binanceApiService.getPriceChangeStatistics(symbol, symbols, windowSize, type));
		}
	...
	public NewOrderResponse newOrder(NewOrder order) {
		...
		if (order.getQuoteOrderQty() == null) {
			call = binanceApiService.newOrder(order.getSymbol(), order.getSide(), order.getType(),
					...
					order.getTimestamp(), order.getTrailingDelta());
		} else {
	...
	public NewOCOResponse newOCO(NewOCO oco) {
		return executeSync(binanceApiService.newOCO(oco.getSymbol(), oco.getListClientOrderId(), oco.getSide(),
				oco.getQuantity(), oco.getLimitClientOrderId(), oco.getPrice(), oco.getLimitIcebergQty(), oco.getTrailingDelta(),
